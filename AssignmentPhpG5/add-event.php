...
//Get all value from add-event.html form
...
$currentDate = date("Y-m-d");
        ...
        if(empty($eventName) || empty($eventDesc)){
            ...
            exit('Event Name and Description should not be blank!!');
        }
        
        if($currentDate > $date){
            header('Refresh:3 url=list-event.php');
            exit('Event date could not be in past');
        }
        
        if($start_time >= $end_time){
           header('Refresh:3 url=list-event.php');
            exit('Event start time should not late then Event end time');
           }
...
$sql = "INSERT INTO 
event 
VALUES (null, '{$eventName}', '{$eventDesc}', 
'{$venue}', '{$maxPerson}', '{$date}', '{$start_time}', '{$end_time}', '{$currentDate}' );";
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"/>
 <link href="https://fonts.googleapis.com/css?family=Roboto:300,400,500,700" rel="stylesheet">
    ...
    <style>
      ...
      border-radius: 6px;
      background: #fff;
      box-shadow:10px 7px rgba(217, 217, 217, 0.215);
...
<div class="testbox">
     <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
        ...
        <div class="item">
          ...
          <textarea rows="3" name="eventDesc" ></textarea>
        </div>
